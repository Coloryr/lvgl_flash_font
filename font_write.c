...
uint32_t write_font(uint32_t pos, uint32_t addr, const char *path) {
    ...
    if (res != FR_OK) {
        //lv_label_set_text_fmt(info_label, "no file %s", path);
        while (1);
    }
    ...
    for (;;) {
        re:
        res = f_read(&font_file, data_temp, TEMP_L, &len);
        if (res != FR_OK) {
            //lv_label_set_text_fmt(info_label, "file %s error res:%d", path, res);
            while (1);
        }
        ...
        res = f_read(&font_file, data_temp, TEMP_L, &len1);
        if (res != FR_OK) {
            //lv_label_set_text_fmt(info_label, "file %s error res:%d", path, res);
            while (1);
        }
        ...
        re1:
        //TODO flash_write
        flash_write(data_temp, pos + now, len);
        //TODO flash_read
        flash_read(data_temp1, pos + now, len);
        ...
        //lv_label_set_text_fmt(info_label, "init font %s %d/%d", path, size, now);
    }
    if (now != size) {
        //lv_label_set_text_fmt(info_label, "init font %s error:size check fail", path);
        while (1);
    }

    //lv_label_set_text_fmt(info_label, "init font %s done.", path);
